...
public class SortArray
{
    ...
    void improvedBubbleSort(T[] a)
    {
        //TODO Project1 D
        int leftIndex = 0;
        int lastSwap = 0;// first position to check
        int rightIndex = a.length - 1; // last position to check
        boolean swapOccur = true; // keeps check of where last swap happened

        while (leftIndex < rightIndex && swapOccur)
        {
            swapOccur = false;
            for (int index = leftIndex; index < rightIndex; index++)
            {
    ...
    void shakerImprovedBubbleSort(T[] a)
    {
        ...
        boolean swapOccur = true;
        startStatistics();
        while (leftIndex < rightIndex && swapOccur) // this while loop is to bubble down
        {
            for (int index = leftIndex; index < rightIndex; index++)
            {
                comparisons++;
    ...
    {
        //TODO Project1 A
        for (int i = 0; i < data.length/2; i++) // use swap instead, use swap(i, (a.length - 1) - i) to s;
        {
