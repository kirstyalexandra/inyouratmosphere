...
public class SortArray
{
    ...
    void findNineCandidatePivotIndexes(ArrayList<Integer> indexes, T[] a, int first, int mid, int last, int size)
    {
        ...
        {
            findNineCandidatePivotIndexes(indexes, a, first, first + (mid - first)/2, mid, size); // find smaller
            findNineCandidatePivotIndexes(indexes, a, mid + 1, (mid + 1) + ((last - (mid+1))/2), last, size); // find larger
        }
    ...
    int partition(T[] a, int first, int last)
    {
        ...
        else if (a.length >= 8 && a.length <= 40)
        {
            // use median-of-three pivot selection scheme
        ...
        {
            //pivot should be the median of nine entries
            findNineCandidatePivotIndexes(indexes, a, first, mid, last, a.length);
    ...
    {
        //TODO Project 2
        // create counters based on maxValue
        ...
        for (int number = 0; number < counterArr.length; number++)
        {
            int count = counterArr[number];
            for (int i = 0; i < count; i++)
            {
                a[sortedIndex] = number;
                sortedIndex++;
            }
        }
    } // end countingSort
    ...
    public static void binaryRadixSort(Integer[] a, int maxNumber)
    {
        ...
        int indexBucket0 = 0;
        int indexBucket1 = 0;
        int mask = 1; // to compare a[i] with
        final int NUMBER_OF_BUCKETS = 2;
        Integer temp [] = new Integer[a.length];
        Integer[][] buckets = new Integer[NUMBER_OF_BUCKETS][a.length];
        int numOfPasses = Integer.SIZE - Integer.numberOfLeadingZeros(maxNumber); // determines number of bits
        ...
        for (int pass = 0; pass < numOfPasses; pass++)
        {
            ...
            for (int index = 0; index < a.length; index++)
            {
                if ((a[index] & mask) == 0)
                {
                    buckets[0][indexBucket0] = a[index];
                    indexBucket0++;
                }
                else
                {
                    buckets[1][indexBucket1] = a[index];
                    indexBucket1++;
                }
            }
            mask = mask << 1;
            int t = 0;
            for (int i = indexBucket0; i < a.length; i++)
            {
                buckets[0][i] = buckets[1][t];
                t++;
            }
